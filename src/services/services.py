...
class Parser:
    ...
    def open_token(res: str, doc: TextIO, indx: int, symbol: str, prm: ParserParams) -> None:
        ...
        if "/" in prm.next_token:
            if split_strip(token) == split_strip(prm.next_token):
                if split_strip(token) not in prm.only_values:
    ...
    def close_token(res: str, doc: TextIO, indx: int, symbol: str, prm: ParserParams) -> None:
        ...
        # Поиск предыдущего токена
        previous_left_key = left_key - 1
        while res[previous_left_key] != "<":
            previous_left_key -= 1
        # Предыдущий токен
        previous_token = res[previous_left_key: left_key + 1]
        ...
        if not next_token:
            ...
            if split_strip(prm.end_stack) != split_strip(next_token):
                if split_strip(prm.end_stack) in prm.only_values:
                    ...
                    elif split_strip(prm.end_stack) in prm.stack_for_array:
                        doc.write(f'\n{(len(prm.stack) + 1) * prm.tab}}}\n')
                        doc.write(f'{(len(prm.stack) + 1) * prm.tab}]\n')
                        prm.stack_for_array.remove(split_strip(prm.end_stack))
                        prm.only_values.remove(split_strip(prm.end_stack))

                elif split_strip(next_token) in prm.only_values:
                    if split_strip(next_token) in prm.stack_for_array:
                        if split_strip(previous_token) != split_strip(prm.end_stack):
                            doc.write(f'{(len(prm.stack) + 1) * prm.tab}}}')
            ...
            elif split_strip(prm.end_stack) != split_strip(next_token):
                if split_strip(prm.end_stack) in prm.only_values:
